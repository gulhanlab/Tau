...
def calculate_prob_cn_beta_binomial(df, dispersion=150):
    ...
    def cn_likelihood(row):
        ...
        likelihoods = []
        for y in cn_states:
            # Expected probability of alternate reads for multiplicity state y
            denom = purity * total_cn + (1 - purity) * 2
            p = purity * y / denom
...
def categorize(sample, input_file=None, output_file=None, vaf_scale=None, vaf_scale_small=None,
               scale_margin=0.4, scale_margin_small=0.2, undefined_max=0.15,
               likelihood_threshold=0.015, dispersion=150, df=None, clonal=False):
    ...
    df['depth'] = df.nalt + df.nref
    df['frac_tumor_reads'] = ( (df.total_cn * df.purity) / 
                                     ( df.total_cn * df.purity + (1-df.purity)*2 )
                                 )
    df['vaf_adj'] = df.nalt / (df['frac_tumor_reads'] * df.depth )
    df['vaf_obs'] = df.nalt / df.depth

    if not clonal:
        # Calculate CCF
        df['CCF'] = df['vaf_adj'] * df['total_cn'] / df['best_cn']
        ...
        if undefined_frac > undefined_max and vaf_scale is None and vaf_scale_small is None:
            ...
            for scale in scales:
                df_tmp = categorize(sample, input_file, output_file, vaf_scale=scale, dispersion=dispersion, df=df.copy())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dispersion", type=float, default=150, help="Dispersion for beta-binomial calculation")
    parser.add_argument("--clonal", type-float, default=False, help="Whether all mutations are clonal or not")
    ...
    categorize(
        ...
        likelihood_threshold=args.likelihood_threshold, dispersion=args.dispersion, clonal=args.clonal
    )
