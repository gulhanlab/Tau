...
# =========================================================
# Route / environment I/O
# =========================================================

# Hardcoded default assets; can be overridden via env vars:
#   export TAU_ROUTES_SAGE=/path/to/routes.sage
#   export TAU_MATRICES_H5=/path/to/route_matrices.h5
...
def _default_paths() -> tuple[Path, Path]:
    sage_path = os.environ.get("TAU_ROUTES_SAGE", DEFAULT_ROUTES_SAGE)
    h5_path = os.environ.get("TAU_MATRICES_H5", DEFAULT_MATRICES_H5)
...
def _segment_time_segment(
    ...
    *,
    ...
    try:
        route_keys = _route_keys_for_state(env, self.major_cn, self.minor_cn)  # if you have this helper
    except NameError:
    ...
    # Avoid re-calling prepare_route / building A,b every loop iteration
    ...
    for rk in route_keys:
        ...
        route_cache[rk] = dict(
            A=A, b=b, K=K,
            n_free=int(n_free),
            # Keep these so time_solution can re-derive as needed (we won't replicate its logic here)
    ...
    for N_vec, meta in _counts_iter(self, method=counts):
        if N_vec is None or np.sum(N_vec) <= 0:
            continue
        boot_id = int(meta.get("boot", 0))  # 0 for original, >=1 for bootstrap

        # Per-boot likelihood of EM pi is constant across routes; compute once
        # (we also need projected likelihood per route, computed below)
        # loglik_base already computed under EM pi; that's the same across boots if pi_em fixed.
        # If you later re-EM per bootstrap, move this inside the boot loop.

        for rk in route_keys:
            A = route_cache[rk]["A"]; b = route_cache[rk]["b"]; K = route_cache[rk]["K"]

            if A.size == 0:
                # No constraints: zero distance, no projection
                d_abs = 0.0
                d_rel = 0.0
                N_use = np.asarray(N_vec, float).copy()
                active_idx = []
            else:
                # Project THIS bootstrap’s N_vec and preserve its own total
            ...
            if A.size == 0:
                ...
                if inside_margin < 0 and inside_margin > -1e-10:  # tiny numeric jitter
                    inside_margin = 0.0

            # Likelihoods: base under EM pi (precomputed), projected under proj-mixture
            ...
            # Timing gate: must be prekept on EM counts, and this boot within dist_cut
...
def _genome_time_segments(
    ...
    *,
    ...
    for seg in tqdm(getattr(self, "segments", []), desc="Timing segments"):
        #print(f'Timing segment {getattr(seg, "seg_id", f"{seg.seg_id}")}...')
    ...
    *,
    ...
    for seg in getattr(self, "segments", []):
        ...
        for key, info in seg.timing_result.items():
            ...
            t_draws  = info.get("t_draws", [])           # list of dicts
            draw_ids = info.get("draw_ids", [])          # list of ints
            boot_ids = info.get("boot_ids", [])          # list of ints

            if not t_draws:
                continue

            # sanity: lengths should match; if not, default boot_id=0, draw_id=range
                ...
                for k_idx, s_k in enumerate(cum, 1):
                    rows.append(
                        dict(
                            ...
                            boot_id=int(boot_id),
                            draw_id=int(draw_id),
                            # optional: keep a legacy 'draw' column equal to draw_id
                            draw=int(draw_id),
    ...
    # Summarize by (seg_id, key, time_interval, boot_id) so you can report per-bootstrap CIs if desired.
    qs = sorted(set(ci))
    ...
    # Expand quantile vector to columns
    out_rows = []
...
def _genome_summarize_constraint_violations(self: Genome, sample: str) -> pd.DataFrame:
    ...
    (No recomputation; included for convenience/debugging.)
    """
    rows: list[dict] = []
    for seg in getattr(self, "segments", []):
        if not getattr(seg, "timing_result", None):
