...
def make_time_df(genome):
    ...
    for seg in genome.segments:
        ...
        keys_with_results = [
            k
            for k in distance_sorted_keys
            if len(timing_result.get(k, {}).get("draws", [])) > 0
        ]

        lls = {
            k: next(x for x in timing_result[k]["qc"] if x["loglik"] is not None)[
                "loglik"
            ]
            for k in keys
        }
                            ...
                            pd.DataFrame(
                                {
                                    ...
                                    "num_draws": num_draws,
                                    "distance_from_route_boundary": euclidean_distances[
                                        key
                                    ],
