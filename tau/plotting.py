...
def plot_tau_stack(
    ...
    if prev_chrom is not None:
        ...
        ax.text(
            mid, -0.04, prev_chrom, ha="center", va="top", fontsize=14, clip_on=False
        )
...
def timepoint_cluster_plotting(
    ...
    handles = [
        plt.Line2D([0], [0], color=cluster_colors[cid], alpha=0.7, lw=4)
        for cid in range(num_clusters)
    ]
    labels = [
        f"{cluster_definitions[cid]}, time = {cluster_times[cid]:.2f}"
        for cid in range(num_clusters)
    ]
    fig.legend(
        handles,
...
def segment_cluster_plotting(ax, clustered_genome, segments_ordered, offsets):
    ...
    fig_inset.set_yticks(
        list(cluster_id_order.values()), labels=list(cluster_id_order.keys())
    )
...
def plot_overview(
    ...
    if (
        cluster_times is not None
        and segment_cluster_ids is not None
        and original_times is not None
        and cluster_definitions is not None
    ):
